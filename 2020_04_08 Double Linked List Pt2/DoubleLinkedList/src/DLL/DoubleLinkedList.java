...
public class DoubleLinkedList<T> implements DoubleLinkedListInterface<T> {
    ...
    public T removeBack() {
        ...
        T old_data = (T)tail.getData();
        tail = old_tail.getPreviousNode();
        old_tail = null;
    ...
    public T removeAt(int position) {
        ...
        {
            return removeFront();  // O(1)
        }
        else if ( position >= listCount )
        {
            return removeBack();  // O(1)
        }
        else
        {
            // O(n) due to traversing to find the insertion point in the single linked list
            ...
            for(int i=0; i<position; i++)   // O(n) time performance
            {
                target = target.getNextNode();
            }
            
            target.getPreviousNode().setNextNode( target.getNextNode() ); // part a of whiteboard diagram
            target.getNextNode().setPreviousNode( target.getPreviousNode() ); // part b of whiteboard diagram
            T old_data = (T)target.getData();
            target = null;
            listCount--;
            return old_data;
        }
